...
/**
 * Launches a new coroutine on the BungeeCord Plugin ThreadPool without blocking the current thread and returns a reference to the coroutine as a [Job].
 ...
 * If the context does not have any dispatcher nor any other [ContinuationInterceptor], then [Plugin.bungeeCordDispatcher] is used.
 * The parent job is inherited from a [Plugin.scope] as well, but it can also be overridden
 * with a corresponding [context] element.
 *
 * By default, the coroutine is scheduled on the BungeeCord thread pool which is executed later.
 ...
 *
 * @param context The coroutine context to start. Should almost be always be [Plugin.bungeeCordDispatcher]. Async operations should be
